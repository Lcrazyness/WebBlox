...
(() => {
    ...
    function createObject(data = {}) {
        ...
        const object = {
            ...
            /*
             * "luau" (Lua-like syntax, transpiled to JS),
             * "javascript" (runs as-is), or "blocks"
             * (compiled from blockData below).
             */
            language:
                data.language ||
                "luau",
    ...
    // SCRIPT EDITOR
    //
    // A Roblox-Studio-style code editor: dark theme, monospace,
    // synced line-number gutter, tab-to-indent. Supports two
    // "Code" languages (Luau-lite and JavaScript) plus a
    // Blocks mode for people who don't want to type code.
    // Everything here just edits object.script / .language /
    // .blockData — the actual execution happens in Player at
    // Play time (see Player/player.js).
    // ============================================================

    let scriptEditorEl = null;

    let scriptEditorObjectId = null;


    function buildScriptEditor() {

        if (scriptEditorEl) {
            return scriptEditorEl;
        }
        ...
        panel.innerHTML = `
            ...
            ">
                <div style="display:flex; align-items:center; gap:10px;">
                    <span id="webbloxScriptEditorIcon" style="font-size:15px;">⌘</span>
                    <input id="webbloxScriptEditorName" style="
                ...
                <div id="webbloxScriptEditorTabs" style="display:flex; gap:4px;">
                    <button data-lang="luau" class="webblox-script-tab">Luau</button>
                    <button data-lang="javascript" class="webblox-script-tab">JavaScript</button>
                    <button data-lang="blocks" class="webblox-script-tab">Blocks</button>
                </div>

                <button id="webbloxScriptEditorClose" style="
                    background: none; border: none; color: #999;
                    font-size: 20px; cursor: pointer; line-height: 1;
                ">&times;</button>
            </div>

            <div id="webbloxScriptCodeView" style="
                flex: 1; display: flex; overflow: hidden; position: relative;
            ">
                <div id="webbloxScriptGutter" style="
                    width: 42px; flex-shrink: 0; background: #1a1a1a; color: #5a5a5a;
                    font: 13px/1.55 Consolas, 'Courier New', monospace; text-align: right;
            ...
            ">
                <div id="webbloxBlocksList" style="display:flex; flex-direction:column; gap:8px;"></div>
                <button id="webbloxAddBlock" style="
                    margin-top: 12px; padding: 8px 14px; background: #2a2a2a; color: #ddd;
                    border: 1px solid #3a3a3a; border-radius: 6px; cursor: pointer; font-size: 12px;
                ">+ Add Block</button>
            </div>

            <div style="
                padding: 8px 14px; border-top: 1px solid #2c2c2c; background: #232323;
                display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;
            ">
                <span id="webbloxScriptHint" style="font-size: 11px; color: #777;"></span>
                <button id="webbloxScriptDone" style="
        ...
        style.textContent = `
            .webblox-script-tab {
                ...
                padding: 5px 12px; font-size: 11.5px; cursor: pointer; border-radius: 5px;
            }
            .webblox-script-tab.active {
    ...
    function setScriptLanguage(lang) {
        ...
        const codeView =
            scriptEditorEl.querySelector(
                "#webbloxScriptCodeView"
            );

        const blocksView =
            scriptEditorEl.querySelector(
                "#webbloxScriptBlocksView"
            );
        ...
        if (lang === "blocks") {
            ...
            hint.textContent =
                "Blocks compile to the same script the code editor produces.";
    ...
    // ------------------------------------------------------------
    // Blocks mode — a small, fixed set of composable actions.
    // Each block compiles down to one line of the same script
    // language a Code-mode script would use, so switching
    // between Blocks and Code never loses information going
    // Blocks -> Code (Code -> Blocks isn't attempted; typed
    // code is preserved as-is if you switch back to Blocks
    // without having added any blocks).
    // ------------------------------------------------------------

    const BLOCK_TYPES = {

        onTouched: {
            label: "When this part is touched",
            fields: [],
            compile: () =>
        ...
        wait: {
            label: "Wait (seconds)",
            fields: [
                { key: "seconds", type: "number", default: 1 }
            ],
            compile: b =>
                `    wait(${b.seconds ?? 1})`
        },
        ...
        print: {
            label: "Print message",
            fields: [
                { key: "message", type: "text", default: "Hello!" }
            ],
            compile: b =>
                `    print("${String(b.message ?? "").replace(/"/g, '\\"')}")`
        },

        setColor: {
            label: "Set part color",
            fields: [
                { key: "color", type: "color", default: "#ff0000" }
            ],
            compile: b =>
                `    part.Color = "${b.color ?? "#ff0000"}"`
        },

        moveBy: {
            label: "Move part by (X, Y, Z)",
            fields: [
                { key: "x", type: "number", default: 0 },
                { key: "y", type: "number", default: 1 },
                { key: "z", type: "number", default: 0 }
            ],
            compile: b =>
                `    part.Position = { x = part.Position.x + ${b.x ?? 0}, y = part.Position.y + ${b.y ?? 0}, z = part.Position.z + ${b.z ?? 0} }`
        },

        endTouched: {
            label: "End \"When touched\"",
            fields: [],
            compile: () =>
                "end)"
        }
    ...
    function renderBlocksList() {
        ...
        object.blockData.forEach(
            (block, index) => {
                ...
                Object.entries(BLOCK_TYPES).forEach(
                    ([key, def]) => {
                ...
                select.addEventListener(
                    ...
                    () => {
                        ...
                        BLOCK_TYPES[select.value]
                            .fields.forEach(
                                field => {

                                    object.blockData[index][field.key] =
                                        field.default;
                                }
                            );
    ...
    function compileBlocksToSource(blockData) {
        ...
        /*
         * Every block template refers to a plain "part"
         * variable rather than repeating "script.Parent"
         * everywhere — define it once up top.
         */
    ...
    function insertObject(
        ...
        if (
            ...
            /*
             * Attach the script to whatever part was
             * selected when you inserted it (like
             * inserting a Script into a Part in Roblox
             * Studio). If nothing real is selected, it's
             * a loose Workspace-level script instead.
             */
        ...
        if (
            ...
            /*
             * GUI elements are 2D, positioned on screen
             * as percentages (0-100), not in the 3D world.
             * They live under whichever ScreenGui they're
             * nested in (via parentId) — a lone Frame/
             * TextLabel/TextButton with no ScreenGui
             * ancestor is just attached directly to the
            ...
            objects:
                Array.from(
                    ...
                    object => {
                        ...
                        /*
                         * Blocks compile down to Luau-lite
                         * source right here, so Player only
                         * ever has to deal with two real
                        ...
                        if (
                            clone.type === "Script" ||
                            clone.type === "LocalScript"
                        ) {

                            if (clone.language === "blocks") {

                                clone.compiledScript =
                                    compileBlocksToSource(
                                        clone.blockData ||
                                        []
                                    );

                                clone.compiledLanguage =
                                    "luau";
